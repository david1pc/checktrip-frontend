...
export class AirportCitySearchService {
    ...
    directDestinations(codigo:string): Observable<any> {
        ...
        let urlApi:string = 'https://test.api.amadeus.com/v1/airport/direct-destinations'+parametros;
        return this.http.get<any>(urlApi);
    }

    flightOffers(codigoOrigen:string, codigoDestino:string, fechaSalida:string, cantidadAdultos: string, clase:string): Observable<any> {
        ...
        let urlApi:string = 'https://test.api.amadeus.com/v2/shopping/flight-offers'+parametros;
        return this.http.get<any>(urlApi);
    }
}
