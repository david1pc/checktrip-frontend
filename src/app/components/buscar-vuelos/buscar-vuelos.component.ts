...
export class BuscarVuelosComponent implements OnInit {
  ...
  constructor(private route: ActivatedRoute, private formBuider: FormBuilder, private services: AirportCitySearchService){
  ...
  ngOnInit(): void {
    this.route.paramMap.subscribe(params => {
      ...
      this.formulario.get('destinationLocationCode')?.setValue(this.codigoDestino);
    });
  }
  ...
  consultar(){
    ...
    if(this.formulario.valid){
      ...
      this.services.flightOffers(value.originLocationCode, value.destinationLocationCode, value.departureDate, value.adults, value.travelClass).subscribe({
        next: (response) => {
