...
export class ModalElegirCiudadesComponent implements OnInit {
  ...
  constructor(private formBuider: FormBuilder, public modal: NgbActiveModal, 
    private services: AirportCitySearchService, private router: Router){
  ...
  buscarVuelos(){
    if(this.formulario.valid){
      this.modal.dismiss();
      this.router.navigate(['/buscarVuelos', this.data.codigo,this.data.nombre, this.formulario.value.codigoDestino, this.formulario.value.nombreDestino]);
    }
  }
}
